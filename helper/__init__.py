...
@cached(ttl=120)
async def get_pug_price():
    return (await get_pug_market_data())["current_price"]["usd"]


@cached(ttl=300)
async def get_pug_market_data():
    global FLOOD_DETECTED
    global CACHE
    if not FLOOD_DETECTED or time.time() - FLOOD_DETECTED > 300:
        _logger.debug("Querying CoinGecko now.")
        async with aiohttp.ClientSession() as session:
            async with session.get("https://api.coingecko.com/api/v3/coins/pugdag", timeout=10) as resp:
                if resp.status == 200:
                    FLOOD_DETECTED = False
