...
class HalvingResponse(BaseModel):
    ...
    nextHalvingAmount: float = 6


@app.get("/info/halving", response_model=HalvingResponse | str, tags=["Pugdag network info"])
async def get_halving(field: str | None = None):
    ...
    daa_list = sorted(HALVING_TABLE)

    for i, to_break_score in enumerate(daa_list):
        if daa_score < to_break_score:
            future_reward = HALVING_TABLE[daa_list[i + 1]]
