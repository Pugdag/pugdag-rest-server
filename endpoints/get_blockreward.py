...
from helper.halving_table import HALVING_TABLE
...
@app.get("/info/blockreward", response_model=BlockRewardResponse | str, tags=["Pugdag network info"])
async def get_blockreward(stringOnly: bool = False):
    """
    Returns the current blockreward in PUG/block
    """
    ...
    for to_break_score in sorted(HALVING_TABLE):
        reward = HALVING_TABLE[to_break_score]
