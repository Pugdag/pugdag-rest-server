...
class UtxoResponse(BaseModel):
    address: str = "pugdag:qrzk988gtanp3nf76xkpexwud5cxfmfygqf42hz38pwea74s6qrj75jee85nj"
    outpoint: OutpointModel
    utxoEntry: UtxoModel


@app.get("/addresses/{pugdagAddress}/utxos", response_model=List[UtxoResponse], tags=["Pugdag addresses"])
async def get_utxos_for_address(pugdagAddress: str = Path(
    description="Pugdag address as string e.g. pugdag:qqkqkzjvr7zwxxmjxjkmxxdwju9kjs6e9u82uh59z07vgaks6gg62v8707g73",
    regex="^pugdag\:[a-z0-9]{61,63}$")):
    """
    Lists all open utxo for a given pugdag address
    """
    resp = await pugdagd_client.request("getUtxosByAddressesRequest",
                                       params={
                                           "addresses": [pugdagAddress]
                                       }, timeout=120)
    try:
        return (utxo for utxo in resp["getUtxosByAddressesResponse"]["entries"] if utxo["address"] == pugdagAddress)
    except KeyError:
