...
@app.get("/info/coinsupply", response_model=CoinSupplyResponse, tags=["Pugdag network info"])
async def get_coinsupply():
    """
    Get $PUG coin supply information
    """
...
@app.get("/info/coinsupply/circulating", tags=["Pugdag network info"],
         response_class=PlainTextResponse)
async def get_circulating_coins(in_billion : bool = False):
    """
    Get circulating amount of $PUG token as numerical value
    """
    resp = await pugdagd_client.request("getCoinSupplyRequest")
...
@app.get("/info/coinsupply/total", tags=["Pugdag network info"],
         response_class=PlainTextResponse)
async def get_total_coins():
    """
    Get total amount of $PUG token as numerical value
    """
    resp = await pugdagd_client.request("getCoinSupplyRequest")
