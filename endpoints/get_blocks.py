...
@app.get("/blocks/{blockId}", response_model=BlockModel, tags=["Pugdag blocks"])
async def get_block(response: Response,
                    blockId: str = Path(regex="[a-f0-9]{64}")):
    """
    Get block information for a given block id
    """
    resp = await pugdagd_client.request("getBlockRequest",
                                       params={
                                           "hash": blockId,
    ...
    if "block" in resp["getBlockResponse"]:
        # We found the block in pugdagd. Just use it
        requested_block = resp["getBlockResponse"]["block"]
    else:
        if IS_SQL_DB_CONFIGURED:
            # Didn't find the block in pugdagd. Try getting it from the DB
    ...
    # It's possible that the block from pugdagd does not contain transactions
...
@app.get("/blocks", response_model=BlockResponse, tags=["Pugdag blocks"])
async def get_blocks(response: Response,
    ...
    the pugdagd node. If this is not possible, the database is getting queryied as backup. In this case the response
    ...
    resp = await pugdagd_client.request("getBlocksRequest",
                                       params={
                                           "lowHash": lowHash,
...
@app.get("/blocks-from-bluescore", response_model=List[BlockModel], tags=["Pugdag blocks"])
async def get_blocks_from_bluescore(response: Response,
                                    blueScore: int = 43679173,
                                    includeTransactions: bool = False):
    """
    Lists block beginning from a low hash (block id). Note that this function is running on a pugdagd and not returning
    data from database.
    """
    response.headers["X-Data-Source"] = "Database"
