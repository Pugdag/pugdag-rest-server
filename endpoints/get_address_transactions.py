...
@app.get("/addresses/{pugdagAddress}/transactions",
         response_model=TransactionForAddressResponse,
         response_model_exclude_unset=True,
         tags=["Pugdag addresses"],
         deprecated=True)
@sql_db_only
async def get_transactions_for_address(
        pugdagAddress: str = Path(
            description="Pugdag address as string e.g. "
                        "pugdag:pzhh76qc82wzduvsrd9xh4zde9qhp0xc8rl7qu2mvl2e42uvdqt75zrcgpm00",
    ...
    #
    # WHERE "script_public_key_address" = 'pugdag:qp7d7rzrj34s2k3qlxmguuerfh2qmjafc399lj6606fc7s69l84h7mrj49hu6'
    #
    ...
    async with async_session() as session:
        resp = await session.execute(text(f"""
            ...
            WHERE "script_public_key_address" = :pugdagAddress
			ORDER by transactions.block_time DESC
			LIMIT 500"""),
                                     {'pugdagAddress': pugdagAddress})
...
@app.get("/addresses/{pugdagAddress}/full-transactions",
...
async def get_full_transactions_for_address(
    ...
    async with async_session() as s:
        ...
        tx_within_limit_offset = await s.execute(select(TxAddrMapping.transaction_id)
                                                 .filter(TxAddrMapping.address == pugdagAddress)
                                                 .limit(limit)
                                                 .offset(offset)
...
@app.get("/addresses/{pugdagAddress}/transactions-count",
...
async def get_transaction_count_for_address(
    ...
    async with async_session() as s:
        count_query = select(func.count()).filter(TxAddrMapping.address == pugdagAddress)
